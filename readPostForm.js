...
function handleQueryResponse(response) { //enacted when the query response is returned
	...
	var posts=[]; //make array to contain all post objects

	for (i=0; i<data3.length; i++){
		makeObject(data3[i][0],data3[i][1],data3[i][2],data3[i][3],data3[i][4]); //calls object creation given elements in data array
	}
	...
	function makeObject(date1, title1, rating1, descrition1, image1) { //function to create a post object
		var obj = {
			date: date1,
			showtitle: title1,
			rating: rating1,
	...
	for (i=0; i<posts.length; i++){
		makePost(posts[i]); //add to array of all posts
	}
...
function makePost(obj){ //makes the post oject into an element on the page
	var post = document.createElement("DIV"); //create div
	...
	var showTitle = document.createElement("H2"); //create heading
	showTitle.className= "showTitle";
	var showTitleText = document.createTextNode("Title: "+obj.showtitle);

	var showImg= document.createElement("IMG") //create image
	showImg.src= obj.image;
	showImg.className= "showImg";

	var rating= document.createElement("H1") //create heading
	...
	var description = document.createElement("P"); //create paragraph
	description.className= "description";
	var descriptionText = document.createTextNode("Description: "+obj.showdescription);    

	//time to append the children!!
